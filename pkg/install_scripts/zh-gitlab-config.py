    ...
    try:
        with urllib.request.urlopen(req) as response:
            if response.status == 200:
                print(f"✅ 成功更新 root 密码: {new_password}")
            else:
...
def update_application_settings(gitlab_url, access_token):
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            print(f"✅ 应用设置已更新 allow_local_requests_from_web_hooks_and_services: {resp_data['allow_local_requests_from_web_hooks_and_services']}")
    except urllib.error.HTTPError as e:
        print(f"❌ HTTP 错误: {e.code} - {e.reason}")
        print(e.read().decode())
    except Exception as e:
...
def main():
    ...
    if not auth_token:
        print("❌ 无法获取登录页面 authenticity_token")
        return
    ...
    if not auth_token2:
        print("❌ 无法获取创建 token 页面的 authenticity_token")
        return
